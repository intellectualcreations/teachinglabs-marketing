    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-surface backdrop-blur-2xl">
        <div className="max-w-[1200px] mx-auto px-12 h-[72px] flex items-center justify-between max-md:px-6">
          ...
          {/* Logo — text only, matching v4 */}
          <Link href="/" className="font-heading text-[22px] font-bold text-text-primary no-underline transition-colors duration-400">
            Teaching Labs
          </Link>
          ...
          <ul className="hidden lg:flex items-center gap-8 list-none">
            {[
              ...
              <li key={href}>
                <Link href={href} className={`font-heading text-[14px] font-medium transition-colors duration-300 ${pathname === href ? 'text-gold font-semibold' : 'text-text-secondary hover:text-gold'}`}>
                  {label}
                </Link>
              </li>
            ))}
            ...
            <li className="group relative flex items-end self-stretch">
              <button className={`flex items-center gap-1 font-heading text-[14px] font-medium group-hover:text-gold transition-colors duration-300 cursor-pointer bg-transparent border-0 p-0 ${['/our-story', '/how-it-works', '/pricing', '/see-the-difference'].includes(pathname) ? 'text-gold font-semibold' : 'text-text-secondary'}`}>
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                <div className="bg-surface border border-border rounded-xl py-2 min-w-[200px] shadow-[0_12px_40px_rgba(0,0,0,0.12)]">
                  {[
                    ...
                    <Link key={href} href={href} className="block px-5 py-2.5 text-sm font-medium text-text-secondary hover:bg-gold/[0.08] hover:text-gold transition-colors duration-150">
                      {label}
                    </Link>
                  ))}
                  ...
                  <Link href="/see-the-difference" className="block px-5 py-2.5 text-sm font-medium text-text-secondary hover:bg-gold/[0.08] hover:text-gold transition-colors duration-150">
                    See the Difference
                  </Link>
                </div>
              </div>
            ...
            <li>
              <Link href="/contact" className={`font-heading text-[14px] font-medium transition-colors duration-300 ${pathname === '/contact' ? 'text-gold font-semibold' : 'text-text-secondary hover:text-gold'}`}>
                Contact
              </Link>
            </li>
          </ul>

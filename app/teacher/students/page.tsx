...
export default function StudentsPage() {
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h1 className="font-heading text-2xl font-bold text-text-primary">Students</h1>
        <p className="text-text-secondary text-sm mt-1">
          {filtered.length} of {allStudents.length} students at Lincoln Elementary
        </p>
      </div>
      ...
      <div className="bg-card-bg border border-border rounded-[14px] p-7 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full border-[1.5px] text-sm font-medium cursor-pointer transition-all
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-10">
            ...
            <h3 className="font-heading font-semibold text-sm text-text-primary">No students match your search</h3>
          </div>
        ) : (
          <div className="max-h-[600px] overflow-y-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="text-left px-3.5 py-2.5 text-xs font-semibold text-text-secondary uppercase tracking-wide border-b-2 border-border">
                    First Name
                  </th>
                  <th className="text-left px-3.5 py-2.5 text-xs font-semibold text-text-secondary uppercase tracking-wide border-b-2 border-border">
                    Last Name
              ...
              <tbody>
                {filtered.map((s) => (
                  <tr
                    ...
                    className="cursor-pointer hover:bg-teal/3 border-b border-border last:border-b-0"
                  >
                    ...
                    <td className="px-3.5 py-3 text-sm font-heading text-text-secondary">{s.id}</td>
                    <td className="px-3.5 py-3">
                      <span className="px-2.5 py-0.5 rounded-full bg-navy/8 text-xs font-semibold text-navy">

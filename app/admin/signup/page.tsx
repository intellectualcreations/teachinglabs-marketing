...
import { Buildings, ArrowLeft, Eye, EyeSlash, MagnifyingGlass } from '@phosphor-icons/react';
...
const DEMO_DISTRICTS: Record<string, string[]> = {
  IN: ['Corydon Central Community Schools', 'Indianapolis Public Schools', 'Fort Wayne Community Schools', 'South Harrison Community Schools', 'New Albany Floyd County Schools'],
  TX: ['Houston Independent School District', 'Dallas Independent School District', 'Austin Independent School District', 'San Antonio Independent School District'],
  CA: ['Los Angeles Unified School District', 'San Diego Unified School District', 'San Francisco Unified School District', 'Oakland Unified School District'],
  NY: ['New York City Department of Education', 'Buffalo City School District', 'Rochester City School District', 'Syracuse City School District'],
  FL: ['Miami-Dade County Public Schools', 'Broward County Public Schools', 'Palm Beach County School District', 'Orange County Public Schools'],
};

const ADMIN_TITLES = [
  'Principal', 'Assistant Principal', 'Superintendent', 'Assistant Superintendent',
  'District Tech Director', 'Curriculum Director', 'Department Head', 'Other',
];
  ...
  return (
    <div className="mb-4" ref={wrapRef}>
      <div className="relative">
        <MagnifyingGlass size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted pointer-events-none" />
        <input
          type="text"
          value={value}
        ...
        {open && !disabled && filtered.length > 0 && (
          <div className="absolute top-full left-0 right-0 bg-surface dark:bg-card-bg border border-border border-t-0 rounded-b-xl max-h-56 overflow-y-auto z-50 shadow-lg">
            ...
            {filtered.map((opt) => {
              ...
              return (
                <button key={opt} type="button" onMouseDown={() => { onSelect(opt); setOpen(false); }}
                  ...
                  {value.trim() && idx !== -1 ? (
                    <>{opt.slice(0, idx)}<mark className="bg-teal/20 text-inherit rounded-sm px-0.5">{opt.slice(idx, idx + value.length)}</mark>{opt.slice(idx + value.length)}</>
                  ) : opt}
                </button>
              );
            })}
...
export default function AdminSignupPage() {
  ...
  const [districtMode, setDistrictMode] = useState<'public' | 'other'>('public');
  ...
  const districtOptions = selectedState && DEMO_DISTRICTS[selectedState] ? DEMO_DISTRICTS[selectedState] : [];
    ...
    <div className="min-h-screen bg-warm-white dark:bg-[#0F1419] flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-[460px] mb-4">
        {screen === 1 ? (
          ...
          <button onClick={() => { setScreen(1); window.scrollTo(0,0); }} className="inline-flex items-center gap-1.5 text-sm text-text-secondary hover:text-text-primary transition-colors">
            <ArrowLeft weight="bold" size={16} />
            Back
          </button>
        )}
      ...
      <div className="flex items-center gap-2 mb-8">
        {[1, 2, 3].map((dot) => (
          <div key={dot} className="rounded-full transition-all duration-300" style={{
            ...
            background: dot <= screen ? '#D97706' : '#E2E8F0',
          }} />
        ))}
      </div>
      ...
      <div className="w-full max-w-[460px] animate-[fadeUp_0.4s_ease-out]">
        ...
        {screen === 1 && (
          <div className="animate-[fadeUp_0.3s_ease-out]">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <input type="text" value={firstName}
                  ...
                  onBlur={(e) => { const v = e.target.value.trim(); if (v) setFirstName(v.charAt(0).toUpperCase() + v.slice(1).toLowerCase()); }}
                  placeholder="Sarah"
                  className={`w-full px-4 py-3 rounded-xl border bg-surface dark:bg-card-bg text-text-primary placeholder:text-text-muted text-sm outline-none focus:ring-2 focus:ring-teal/30 transition-all ${firstNameError ? 'border-danger' : 'border-border focus:border-teal'}`}
                />
              </div>
              <div>
                <label className="block font-heading text-sm font-medium text-text-primary mb-1.5">
                ...
                <input type="text" value={lastName}
                  ...
                  onBlur={(e) => { const v = e.target.value.trim(); if (v) setLastName(v.charAt(0).toUpperCase() + v.slice(1).toLowerCase()); }}
                  placeholder="Williams"
                  className={`w-full px-4 py-3 rounded-xl border bg-surface dark:bg-card-bg text-text-primary placeholder:text-text-muted text-sm outline-none focus:ring-2 focus:ring-teal/30 transition-all ${lastNameError ? 'border-danger' : 'border-border focus:border-teal'}`}
                />
              </div>
            </div>
            ...
            {/* Title */}
            <div className="mb-5">
              <label className="block font-heading text-sm font-medium text-text-primary mb-1.5">Title / Role</label>
              <select value={title} onChange={(e) => setTitle(e.target.value)}
                className="w-full px-4 py-3 rounded-xl border border-border bg-surface dark:bg-card-bg text-text-primary text-sm outline-none focus:ring-2 focus:ring-teal/30 focus:border-teal transition-all appearance-none cursor-pointer"
                ...
                {ADMIN_TITLES.map((t) => <option key={t}>{t}</option>)}
              </select>
            </div>

            {/* Find district */}
            <div onClick={() => setDistrictMode('public')}
              className={`border-[1.5px] rounded-2xl p-5 mb-3 cursor-pointer transition-all duration-200 ${districtMode === 'public' ? 'border-navy bg-card-bg dark:bg-[#1A2332] shadow-[0_0_0_1px_#1F3A5F]' : 'border-border hover:border-teal bg-surface dark:bg-card-bg'}`}>
              <div className="flex items-center gap-3 mb-1">
                <div className="w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors"
                  style={{ borderColor: districtMode === 'public' ? '#1F3A5F' : '#E2E8F0' }}>
                  {districtMode === 'public' && <div className="w-2.5 h-2.5 rounded-full bg-navy" />}
                </div>
                <span className="font-heading font-semibold text-[15px] text-text-primary">Find your district</span>
              </div>
              <p className="text-[13px] text-text-secondary ml-8 mb-0">Search our national database of public school districts</p>

              {districtMode === 'public' && (
                <div className="mt-4" onClick={(e) => e.stopPropagation()}>
                  <label className="block font-heading text-sm font-medium text-text-primary mb-1.5">State</label>
                  <select value={selectedState}
                    onChange={(e) => { setSelectedState(e.target.value); setDistrictQuery(''); setSelectedDistrict(''); }}
                  ...
                  <Autocomplete
                    ...
                    onSelect={(val) => { setSelectedDistrict(val); setDistrictQuery(val); }}
            ...
            {/* Can't find district */}
            <div onClick={() => setDistrictMode('other')}
              className={`border-[1.5px] rounded-2xl p-5 mb-4 cursor-pointer transition-all duration-200 ${districtMode === 'other' ? 'border-navy bg-card-bg dark:bg-[#1A2332] shadow-[0_0_0_1px_#1F3A5F]' : 'border-border hover:border-teal bg-surface dark:bg-card-bg'}`}>
              <div className="flex items-center gap-3 mb-1">
                <div className="w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors"
                  style={{ borderColor: districtMode === 'other' ? '#1F3A5F' : '#E2E8F0' }}>
                  {districtMode === 'other' && <div className="w-2.5 h-2.5 rounded-full bg-navy" />}
                </div>
                <span className="font-heading font-semibold text-[15px] text-text-primary">Can&apos;t find your district?</span>
              </div>
              <p className="text-[13px] text-text-secondary ml-8">Add your district or organization manually</p>

              {districtMode === 'other' && (
                <div className="mt-4" onClick={(e) => e.stopPropagation()}>
                  <label className="block font-heading text-sm font-medium text-text-primary mb-1.5">Zip code</label>
                  <input type="text" value={otherZip}
                    onChange={(e) => setOtherZip(e.target.value.replace(/\D/g,'').slice(0,5))}
                    placeholder="e.g. 47112" maxLength={5}
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div>
                      ...
                      <input type="text" value={otherCity} onChange={(e) => setOtherCity(e.target.value)} placeholder="Auto-fills from zip"
                    ...
                    <div>
                      <label className="block font-heading text-sm font-medium text-text-primary mb-1.5">State</label>
                      <input type="text" value={otherStateDisplay} onChange={(e) => setOtherStateDisplay(e.target.value)} placeholder="Auto-fills from zip"
                        className="w-full px-4 py-3 rounded-xl border border-border bg-bg-secondary dark:bg-[#1E2A3A] text-text-primary text-sm outline-none" />
                    </div>
                  </div>
                  <label className="block font-heading text-sm font-medium text-text-primary mb-1.5">District or organization name</label>
                  <input type="text" value={otherDistrictName} onChange={(e) => setOtherDistrictName(e.target.value)}
                    placeholder="e.g. Springfield Public Schools"
                    className="w-full px-4 py-3 mb-4 rounded-xl border border-border bg-surface dark:bg-card-bg text-text-primary text-sm outline-none focus:ring-2 focus:ring-teal/30 focus:border-teal transition-all" />
                  <label className="block font-heading text-sm font-medium text-text-primary mb-1.5">Organization type</label>
                  <select value={otherOrgType} onChange={(e) => setOtherOrgType(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl border border-border bg-surface dark:bg-card-bg text-text-primary text-sm outline-none focus:ring-2 focus:ring-teal/30 focus:border-teal transition-all appearance-none cursor-pointer"
                    style={{ backgroundImage: SELECT_ARROW, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 14px center', paddingRight: 36 }}>
                    <option value="">Select type</option>
                    <option value="public">Public School District</option>
                    <option value="private">Private School Network</option>
                    <option value="charter">Charter Network</option>
                    <option value="diocese">Diocese / Religious Network</option>
          ...
          <div className="animate-[fadeUp_0.3s_ease-out]">
            ...
            <div className="flex flex-col gap-3 mb-6">
              {[
                ...
                <button key={label} onClick={handleCreate}
            ...
            <div className="mb-4">
              <label className="block font-heading text-sm font-medium text-text-primary mb-1.5">Work email</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@district.edu"
